...
const STATUS_COLORS: Record<TaskStatus, string> = {
  open: 'var(--color-accent)', done: '#1D9E75', cancelled: 'var(--sb-ink-3)',
}
...
const lbl: React.CSSProperties = {
  fontSize: 10.5, color: 'var(--sb-ink-3)', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 4,
}
...
function activityIcon(type: string) {
  ...
  return                                <span style={{ ...s, background: 'var(--sb-ink-3)' }} />
}
...
export function TaskDetailModal({ task, onClose }: Props) {
  ...
  const companyColor = activeCo?.color ?? COMPANY_COLORS[task.company] ?? 'var(--sb-ink-3)'
      ...
      >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {editingTitle ? (
              ...
              <h2
                ...
                style={{
                  margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--sb-ink-1)',
                  cursor: 'text', lineHeight: 1.3,
                  textDecoration: task.completed ? 'line-through' : 'none',
                }}
              >{task.title}</h2>
            )}
            <div style={{ marginTop: 4, fontSize: 11, color: 'var(--sb-ink-3)' }}>
              Created {relativeTime(task.createdAt)}
            </div>
          </div>
          <button onClick={onClose} style={{
            background: 'transparent', border: 'none', cursor: 'pointer',
            color: 'var(--sb-ink-3)', padding: 4, borderRadius: 6, flexShrink: 0,
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px' }}>
          ...
          <div>
            ...
            {taskActs.length === 0 ? (
              <p style={{ fontSize: 12, color: 'var(--sb-ink-3)', fontStyle: 'italic', margin: 0 }}>No activity yet.</p>
            ) : (
                ...
                {taskActs.map(a => (
                  <div key={a.id} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <span style={{ fontSize: 10, color: 'var(--sb-ink-3)', marginLeft: 8 }}>{relativeTime(a.timestamp)}</span>
                    </div>
                  </div>
                ))}
              </div>
