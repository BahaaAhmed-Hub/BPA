...
const STATUS_COLORS: Record<TaskStatus, string> = {
  open: '#7F77DD', done: '#1D9E75', cancelled: '#6C6553',
}

const field: React.CSSProperties = {
  background: '#F7F4EA', border: '1px solid #E8E1CE', borderRadius: 6,
  padding: '6px 9px', fontSize: 12, color: '#191712', outline: 'none', width: '100%',
  fontFamily: 'inherit', boxSizing: 'border-box',
}
const lbl: React.CSSProperties = {
  fontSize: 10.5, color: '#6C6553', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 4,
}
...
function activityIcon(type: string) {
  ...
  return                                <span style={{ ...s, background: '#6C6553' }} />
}
...
export function TaskDetailModal({ task, onClose }: Props) {
  ...
  const companyColor = activeCo?.color ?? COMPANY_COLORS[task.company] ?? '#6C6553'
    ...
    >
      ...
      >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {editingTitle ? (
              ...
              <h2
                ...
                style={{
                  margin: 0, fontSize: 16, fontWeight: 700, color: '#191712',
                  cursor: 'text', lineHeight: 1.3,
                  textDecoration: task.completed ? 'line-through' : 'none',
                }}
              >{task.title}</h2>
            )}
            <div style={{ marginTop: 4, fontSize: 11, color: '#6C6553' }}>
              Created {relativeTime(task.createdAt)}
            </div>
          </div>
          <button onClick={onClose} style={{
            background: 'transparent', border: 'none', cursor: 'pointer',
            color: '#6C6553', padding: 4, borderRadius: 6, flexShrink: 0,
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px' }}>
              ...
              <div style={{ display: 'flex', gap: 5 }}>
                {(['open', 'done', 'cancelled'] as TaskStatus[]).map(s => (
                  <button key={s} onClick={() => setStatus(task.id, s)} style={{
                    ...
                    border: `1px solid ${taskStatus === s ? STATUS_COLORS[s] + '80' : '#E8E1CE'}`,
                    color: taskStatus === s ? STATUS_COLORS[s] : '#6C6553',
                  }}>{s}</button>
                ))}
              </div>
            </div>
          ...
          <div>
            <div style={{
              fontSize: 11, fontWeight: 700, color: '#6C6553',
              letterSpacing: '0.06em', textTransform: 'uppercase',
              marginBottom: 10,
            }}>Activity</div>

            {taskActs.length === 0 ? (
              <p style={{ fontSize: 12, color: '#6C6553', fontStyle: 'italic', margin: 0 }}>No activity yet.</p>
            ) : (
