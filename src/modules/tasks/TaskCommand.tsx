              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '2px 0 10px' }}>
                  ...
                  <span style={{ marginLeft: 'auto' }}>
                    <div onClick={() => setHideCompleted(!hideCompleted)} style={{ width: 38, height: 22, boxSizing: 'border-box', borderRadius: 'var(--sb-r-pill)', background: hideCompleted ? 'var(--sb-ink-1)' : 'var(--sb-border)', display: 'flex', alignItems: 'center', padding: 2, justifyContent: hideCompleted ? 'flex-end' : 'flex-start', cursor: 'pointer', transition: 'all .15s' }}>
                      <div style={{ width: 18, height: 18, borderRadius: 'var(--sb-r-pill)', background: 'var(--sb-card)' }} />
                    </div>
                  </span>
                </div>
      ...
      {viewMode === 'list' ? (
        ...
        <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          {/* 9F: the brain dump rail sits to the LEFT of the matrix.
              The rail sizes itself (44 px collapsed, clamp(240–360px) expanded).
              The board takes all remaining space so that DO + PLAN + the detail
              panel fill the full page width regardless of whether the dump is
              open or shut. */}
          <div style={{ display: 'flex', gap: 16, padding: '4px 28px 28px', alignItems: 'flex-start' }}>
            ...
            {modalTask && (
              <div data-task-node style={{ paddingBottom: 28 }}>
                <TaskDetailPanel key={modalTask.id} task={modalTask} onClose={() => setModalTaskId(null)} />
              </div>
            )}
          </div>
