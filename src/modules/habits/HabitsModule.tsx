        ...
        return (
          <div
            ...
            style={{
              ...
              background: done ? color : '#1C1814',
              border: `1px solid ${done ? color : isToday ? '#5A4E3A' : '#3A3020'}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}
          >
            <span style={{ fontSize: 8.5, color: done ? '#1C1814' : '#4A3E28', fontWeight: 600 }}>
              {fmtDayLabel(d)}
            </span>
          </div>
        )
  ...
  return (
    <div>
      ...
      <div style={{ padding: '28px 28px 60px' }}>
          ...
          {[
            {
              ...
              color: completionRate === 100 ? '#1D9E75' : '#C49A3C',
            },
            {
              label: 'Best Streak',
            ...
            <div key={card.label} style={{
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: '#F0E8D8', fontFamily: "'Cabinet Grotesk', sans-serif", letterSpacing: '-0.5px', lineHeight: 1 }}>
                {card.value}
              </div>
              <div style={{ fontSize: 12.5, color: '#8A7A60', marginTop: 4 }}>{card.label}</div>
        ...
        {/* ─── Habits list ─────────────────────────────────────────────────── */}
        <div style={{ background: '#2A2218', border: '1px solid #3A3020', borderRadius: 14, overflow: 'hidden', marginBottom: 14 }}>

          {/* List header */}
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            padding: '16px 20px', borderBottom: '1px solid #3A3020',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: '#8A7A60', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                Habit
              </span>
              <div style={{ display: 'flex', gap: 4 }}>
                {days.map(d => (
                  <div key={d} style={{ width: 22, textAlign: 'center' }}>
                    <span style={{ fontSize: 9.5, color: d === today ? '#C49A3C' : '#5A4E3A', fontWeight: d === today ? 600 : 400 }}>
                      {fmtDayLabel(d)}
                    </span>
                  </div>
                ))}
          ...
          {habits.length === 0 && (
            <div style={{ padding: '32px', textAlign: 'center' }}>
              <p style={{ margin: 0, fontSize: 13, color: '#8A7A60' }}>No habits yet. Add one below.</p>
            </div>
          )}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: '14px 20px',
                  borderBottom: i < habits.length - 1 ? '1px solid #3A3020' : 'none',
              ...
              >
                ...
                <span style={{
                  ...
                  color: doneToday ? habit.color : '#F0E8D8',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, width: 52, justifyContent: 'flex-end' }}>
                  {streak > 0 && <Flame size={12} color={streak >= 7 ? '#E05252' : '#C49A3C'} />}
                  <span style={{ fontSize: 13, fontWeight: 600, color: streak > 0 ? (streak >= 7 ? '#E05252' : '#C49A3C') : '#5A4E3A' }}>
                    {streak > 0 ? `${streak}d` : '—'}
                  </span>
                </div>
        ...
        {addingHabit ? (
          <div style={{
            ...
            <p style={{ margin: '0 0 14px', fontSize: 12, fontWeight: 600, color: '#8A7A60', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
              New Habit
            </p>
            <input
              autoFocus
              ...
              style={{
                ...
                background: '#1C1814', border: '1px solid #4A3E28',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 7, background: 'transparent', border: '1px solid #3A3020', color: '#8A7A60', fontSize: 12, cursor: 'pointer' }}
                >
                  <X size={12} /> Cancel
                </button>
                <button
          ...
          <button
            ...
            style={{
              ...
              background: 'transparent', border: '1px dashed #3A3020',
              color: '#5A4E3A', fontSize: 13, cursor: 'pointer',
              transition: 'all 0.15s',
            }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#C49A3C50'; (e.currentTarget as HTMLElement).style.color = '#C49A3C' }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#3A3020'; (e.currentTarget as HTMLElement).style.color = '#5A4E3A' }}
          >
