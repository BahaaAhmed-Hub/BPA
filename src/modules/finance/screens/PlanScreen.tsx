    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 20, flexShrink: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          <span style={{ fontSize: 'var(--sb-t-meta)', fontWeight: 700, letterSpacing: '0.14em', color: C.ink3 }}>FINANCE · PLAN</span>
          <span style={{ fontFamily: 'var(--sb-font-num)', fontSize: 'var(--sb-t-display)', fontWeight: 600, letterSpacing: '-0.03em', lineHeight: 1, color: C.ink1 }}>Settle &amp; achieve</span>
          <span style={{ fontSize: 'var(--sb-t-body-s)', color: C.ink3, paddingTop: 3 }}>Built from the forecast: what to pay, from which account, on which date — and what it frees up next</span>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 11 }}>
          ...
          <div style={{ flexShrink: 0, background: C.dark, color: 'var(--sb-ink-on-dark)', borderRadius: 'var(--sb-r-card)', padding: '14px 18px', display: 'flex', gap: 20, alignItems: 'center' }}>
            ...
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                <span style={{ fontSize: 'var(--sb-t-micro)', fontWeight: 700, letterSpacing: '0.12em', opacity: 0.55 }}>THEN THE FREED EGP 20,000 A MONTH GOES TO UMRAH</span>
                <span style={{ marginLeft: 'auto', fontSize: 'var(--sb-t-micro)', opacity: 0.6, flexShrink: 0 }}>target met 21 November — three months early</span>
              </div>
            ...
            <div style={{ width: 140, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 9 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                <span style={{ fontFamily: 'var(--sb-font-num)', fontSize: 'var(--sb-t-h2)', fontWeight: 600, letterSpacing: '-0.03em', fontVariantNumeric: 'tabular-nums' }}>EGP 15,000</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 'var(--sb-r-card)', padding: '15px 17px', display: 'flex', flexDirection: 'column', gap: 11, boxSizing: 'border-box', flex: 1, minHeight: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 11, flexShrink: 0 }}>
              ...
              <span style={{ marginLeft: 'auto', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5, fontSize: 'var(--sb-t-meta)', fontWeight: 600, color: C.ink1 }}>
                Sep 2026 → Feb 2027
            ...
            <div style={{ flex: 1, minHeight: 0, border: `1px solid ${C.border}`, borderRadius: 'var(--sb-r-nav)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              ...
              {DEMO_PLAN.map((row, _idx) => (
                <div key={row.month} style={{ display: 'grid', gridTemplateColumns: '64px 1fr 1fr 1fr 1fr 118px 84px 1fr', height: 36, flexShrink: 0, borderTop: `1px solid #F2EDDF`, background: row.rowBg }}>
                  ...
                  {/* Committed */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 9px', fontFamily: 'var(--sb-font-num)', fontSize: 'var(--sb-t-meta)', fontWeight: 500, color: C.ink2, fontVariantNumeric: 'tabular-nums' }}>{fmt(row.committed)}</div>
                  {/* Free Cash */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 9px', fontFamily: 'var(--sb-font-num)', fontSize: 'var(--sb-t-meta)', fontWeight: 600, color: C.ink1, fontVariantNumeric: 'tabular-nums' }}>{fmt(row.freeCash)}</div>
                  {/* To Card */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 9px', fontFamily: 'var(--sb-font-num)', fontSize: 'var(--sb-t-meta)', fontWeight: 600, color: row.toCard ? C.rustDk : C.muted, fontVariantNumeric: 'tabular-nums' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 9px', minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: 'var(--sb-t-meta)', color: row.from !== '—' ? C.ink2 : C.muted, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{row.from}</span>
                  </div>
                  {/* On */}
                  <div style={{ display: 'flex', alignItems: 'center', padding: '0 9px' }}>
                    <span style={{ fontSize: 'var(--sb-t-meta)', fontWeight: row.on !== '—' ? 600 : 500, color: row.on !== '—' ? C.ink1 : C.muted }}>{row.on}</span>
                  </div>
                  {/* Card Left */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 7, padding: '0 9px' }}>
                    {row.closedRow ? (
                      ...
                      <span style={{ fontFamily: 'var(--sb-font-num)', fontSize: 'var(--sb-t-body-s)', fontWeight: 600, color: row.cardLeft === 0 ? C.olive : C.ink1, fontVariantNumeric: 'tabular-nums' }}>{row.cardLeft === 0 ? '0' : fmt(row.cardLeft)}</span>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div style={{ width: 346, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 11 }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 'var(--sb-r-card)', padding: '15px 17px', display: 'flex', flexDirection: 'column', gap: 11, boxSizing: 'border-box', flexShrink: 0 }}>
                ...
                return (
                  <button key={s.id} onClick={() => setStrategy(s.id)} style={{
                    ...
                    <span style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                      <span style={{ fontSize: 'var(--sb-t-body-s)', fontWeight: 600, color: C.ink1 }}>{s.label}</span>
                      <span style={{ fontSize: 'var(--sb-t-micro)', color: C.ink3 }}>{s.sub}</span>
                    </span>
                  </button>
                )
              })}
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 11px', borderRadius: 'var(--sb-r-nav)', background: C.field, border: `1px solid ${C.border}` }}>
              ...
              <span style={{ fontSize: 'var(--sb-t-meta)', color: C.ink2, lineHeight: 1.35 }}>The instalment card charges no interest while the plans run, so paying it early buys nothing. Leave it.</span>
            </div>
          </div>
            ...
            <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 7 }}>
              {automations.map(a => (
                <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10, height: 42, flexShrink: 0, padding: '0 12px', borderRadius: 'var(--sb-r-nav)', background: C.field, border: `1px solid ${C.border}`, boxSizing: 'border-box' }}>
                  ...
                  <span style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                    <span style={{ fontSize: 'var(--sb-t-body-s)', fontWeight: 600, color: C.ink1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.label}</span>
                    <span style={{ fontSize: 'var(--sb-t-micro)', color: C.ink3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.sub}</span>
                  </span>
                  {/* Toggle */}
