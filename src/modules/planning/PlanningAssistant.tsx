...
function analyzeEvents(events: GCalEvent[], weekDays: Date[]): Insight[] {
  ...
  for (const day of byDay) {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      ...
      if (startB - endA < 15 * 60 * 1000) btbCount++
    }
  }
  ...
  const morningFree = byDay.filter(d => !d.some(e => {
    ...
    const h = s ? parseInt(s.slice(11, 13), 10) : 99
    return h >= 7 && h < 11
  }))
...
function timeToMin(dateTime: string): number {
  ...
  const m = parseInt(dateTime.slice(14, 16), 10)
  return h * 60 + m
}
...
function getEventColor(_event: GCalEvent, index: number): string {
...
function WeekCalendar({ events, weekStart, viewMode, selectedDay, onSelectDay }: WeekCalendarProps) {
        ...
        {visibleDays.map((d, i) => {
          ...
          return (
            <div key={i} onClick={() => onSelectDay(days.indexOf(d))} style={{
              ...
              <div style={{ fontSize: 11, color: 'var(--color-text-muted, #6B7280)', fontWeight: 600, letterSpacing: '0.04em' }}>{dayName}</div>
              <div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex' }}>
        ...
        {visibleDays.map((d, di) => {
          ...
          const dayEvents = events.filter(e => {
            const s = e.start?.dateTime ?? e.start?.date ?? ''
            return s.startsWith(ds) && e.start?.dateTime  // only timed events
          }).sort((a, b) => (a.start!.dateTime! > b.start!.dateTime! ? 1 : -1))
...
export function PlanningAssistant() {
  ...
  async function sendChat() {
    ...
    try {
      ...
      const prompt = `The user wants help planning their schedule. Their calendar this week has ${events.length} events. Insights: ${insights.map(i => i.message).join('; ')}. User says: "${q}". Respond with actionable scheduling advice in 2-3 sentences.`
      const reply = await call(system, prompt)
      setMessages(prev => [...prev, { role: 'assistant', text: reply }])
    } catch {
      setMessages(prev => [...prev, { role: 'assistant', text: 'I couldn\'t analyze your schedule right now. Please check your calendar connection.' }])
    } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden' }}>
        {/* ── Left panel ── */}
        <div style={{ width: 380, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--color-border, #252A3E)', overflowY: 'auto', padding: '20px 20px' }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {loading ? (
              ...
              <>
                ...
                {insights.length > 0 && <p style={{ margin: '8px 0 0', fontSize: 11, color: 'var(--color-text-muted, #6B7280)', fontStyle: 'italic' }}>Click an insight to get help fixing it</p>}
              </>
            )}
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <p style={{ margin: '0 0 12px', fontSize: 12, color: 'var(--color-text-muted, #6B7280)', lineHeight: 1.6 }}>
              Describe your ideal schedule and I'll suggest events. Preview changes before applying.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {WEEK_SUGGESTIONS.map((s, i) => (
                <button key={i} onClick={() => setChatInput(s)} style={{
                  ...
                  color: 'var(--color-text-dim, #C0C4D6)', transition: 'background 0.12s', fontWeight: 400,
                }}>
                  {s}
                </button>
              ))}
          ...
          {/* Chat messages */}
          {messages.length > 0 && (
            <div ref={chatRef} style={{ flex: 1, overflowY: 'auto', marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 200 }}>
              {messages.map((m, i) => (
                <div key={i} style={{
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          {/* Calendar grid */}
          <WeekCalendar events={events} weekStart={weekStart} viewMode={viewMode} selectedDay={selectedDay} onSelectDay={setSelectedDay} />
        </div>
      </div>
    </div>
  )
