...
const inp: React.CSSProperties = {
  background: 'var(--color-bg,#F7F4EA)', border: '1px solid #E8E1CE',
...
export function Step4Habits({ data, onChange }: Props) {
  ...
  return (
    <div>
        ...
        {HABIT_TEMPLATES.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => toggleTemplate(t.id)} style={{
              ...
              border: `1.5px solid ${selected ? t.color : '#E8E1CE'}`,
      ...
      {data.customHabits.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 }}>
          {data.customHabits.map((h, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 8, background: 'var(--color-surface,#FFFFFF)', border: '1px solid #E8E1CE' }}>
      ...
      {addingCustom ? (
        <div style={{ padding: 16, borderRadius: 10, background: 'var(--color-surface,#FFFFFF)', border: '1px solid #E8E1CE' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            <button onClick={() => setCType('boolean')} style={{ flex: 1, padding: '7px', borderRadius: 7, border: `1.5px solid ${cType === 'boolean' ? '#F5D14E' : '#E8E1CE'}`, background: cType === 'boolean' ? 'rgba(127,119,221,0.1)' : 'transparent', color: 'var(--color-text,#E8EAF6)', fontSize: 12.5, cursor: 'pointer' }}>
              ✓ Boolean
            </button>
            <button onClick={() => setCType('quantity')} style={{ flex: 1, padding: '7px', borderRadius: 7, border: `1.5px solid ${cType === 'quantity' ? '#F5D14E' : '#E8E1CE'}`, background: cType === 'quantity' ? 'rgba(127,119,221,0.1)' : 'transparent', color: 'var(--color-text,#E8EAF6)', fontSize: 12.5, cursor: 'pointer' }}>
              # Quantity
            </button>
          </div>
          {cType === 'quantity' && (
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            {(['daily','weekdays','weekly'] as const).map(f => (
              <button key={f} onClick={() => setCFreq(f)} style={{ flex: 1, padding: '6px', borderRadius: 7, border: `1.5px solid ${cFreq === f ? '#F5D14E' : '#E8E1CE'}`, background: cFreq === f ? 'rgba(127,119,221,0.1)' : 'transparent', color: 'var(--color-text,#E8EAF6)', fontSize: 12, cursor: 'pointer', textTransform: 'capitalize' }}>
                {f}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={addCustom} style={{ padding: '8px 18px', borderRadius: 8, background: '#F5D14E', color: '#fff', fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer' }}>Add</button>
