        ...
        <div>
          {editable && onChange ? (
            <input type="number" min={0} max={168} value={value} onChange={e => onChange(parseFloat(e.target.value) || 0)}
              style={{ fontSize: 28, fontWeight: 700, color: '#191712', fontFamily: "'Cabinet Grotesk', sans-serif", letterSpacing: '-0.5px', background: 'none', border: 'none', outline: 'none', width: 80, padding: 0 }} />
          ) : (
...
function PillStat({ done, total, label, color }: { done: number; total: number; label: string; color: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <div style={{ height: 4, width: 80, background: '#FFFFFF', borderRadius: 2, overflow: 'hidden' }}>
    ...
    <div style={{ background: '#F7F4EA', border: '1px solid #FFFFFF', borderRadius: 12, padding: '14px 18px', marginBottom: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
        <svg width={136} height={136} style={{ flexShrink: 0 }}>
          {paths.map((p, i) => (
            <path key={i} d={p.d} fill={p.color} stroke="#F7F4EA" strokeWidth={1.5} />
          ))}
        </svg>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 7, minWidth: 130 }}>
          {active.map((sl, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 11, color: '#6C6553', fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>{fmt(sl.minutes)}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div style={{ borderBottom: '1px solid #FFFFFF' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
          ...
          <div style={{ width: 1, height: 12, background: '#E8E1CE' }} />
    ...
    <div>
      ...
      <div style={{ padding: '28px 28px 60px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h2 style={{ margin: '0 0 3px', fontSize: 20, fontWeight: 700, color: '#191712', fontFamily: "'Cabinet Grotesk', sans-serif", letterSpacing: '-0.3px' }}>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E8E1CE', borderRadius: 14, overflow: 'hidden' }}>
          ...
          {/* Navigation header */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '1px solid #E8E1CE' }}>

            <button
              onClick={() => setSelectedDay(d => shiftDay(d, viewMode === 'weekly' ? -7 : -1))}
              style={{ background: 'none', border: '1px solid #E8E1CE', borderRadius: 7, cursor: 'pointer', color: '#6C6553', padding: '5px 8px', display: 'flex', alignItems: 'center' }}
            ><ChevronLeft size={15} /></button>

            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <CalendarDays size={15} color="#7F77DD" />
              <span style={{ fontSize: 14, fontWeight: 600, color: '#191712' }}>
                {viewMode === 'daily'
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {/* Daily / Weekly toggle */}
              <div style={{ display: 'flex', background: '#F7F4EA', border: '1px solid #E8E1CE', borderRadius: 7, overflow: 'hidden' }}>
                {(['daily', 'weekly'] as const).map(mode => (
                  <button
                    ...
                    style={{
                      ...
                      color: viewMode === mode ? '#fff' : '#6C6553',
          ...
          {/* Analytics bar */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 24, padding: '11px 20px', borderBottom: '1px solid #FFFFFF', background: '#F7F4EA' }}>
            {viewMode === 'daily' ? (
              <>
                <PillStat done={doneEvents.length} total={dayEvents.length} label="events done" color="#7F77DD" />
                <div style={{ width: 1, height: 20, background: '#E8E1CE' }} />
