// ─── What kind of thing is this thread — the server's copy ───────────────────────────────────────
//
...
import type { MailHeader, NeutralMessage } from './mailRules.ts'

// Defined here rather than imported: `mailRules.ts` imports this file, and a
// cycle between the two would be a needless thing to reason about.
function headerOf(hs: MailHeader[], name: string): string {
